cat << 'PATCH' > import.patch
--- src/components/ImportStudentData.tsx
+++ src/components/ImportStudentData.tsx
@@ -102,6 +102,23 @@
         const findKey = (keys: string[]) => {
           const foundKey = Object.keys(row).find(k => keys.includes(k));
...
+                day = parts[0].padStart(2, '0'); month = parts[1].padStart(2, '0'); year = parseInt(parts[2], 10);
+              } else if (parts[0].length === 4 && parts[2].length <= 2) {
+                year = parseInt(parts[0], 10); month = parts[1].padStart(2, '0'); day = parts[2].padStart(2, '0');
+              }
+              if (year !== undefined) {
...
+          return parsed;
         ...
         const studentId = findKey(['รหัสนักเรียน', 'เลขประจำตัว', 'studentId', 'id']);
@@ -115,26 +132,9 @@
...
-        }
-        const dob = parsedDob;
+        const dob = parseDate(findKey(['วันเกิด', 'dob', 'วันเดือนปีเกิด']));
+        
...
PATCH
patch src/components/ImportStudentData.tsx import.patch
