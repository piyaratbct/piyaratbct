...
import { Student, SubjectScore, SUBJECTS, SchoolSubject } from "../types";
import {
  PDFPrintHelper,
  PrintPageContainer,
  PrintHeader,
        ...
        if (schoolSubjects.length > 0) {
           ...
           studentScores = parentsAndStandalone.map(subjectDef => {
             if (subjectDef.isParent) {
                ...
                return {
                   subject: \`\${subjectDef.subjectCode} \${subjectDef.name}\`,
                   totalScore: hasAnyScore ? totalScore : "-",
                   grade: hasAnyScore ? grade : "-",
                   isScout: subjectDef.name.includes("ลูกเสือ")
                };
             } else {
...
console.log("Rewrote StudentReportPrintTemplate");
