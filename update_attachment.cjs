...
// add handleImageUpload right before handleAddLink
const newUploadMethod = `  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleAddLink = () => {`;
content = content.replace(`  const handleAddLink = () => {`, newUploadMethod);


// replace the layout string
const oldLayoutStart = `      <div>
...
content = content.replace(oldLayoutStart, newLayout);
fs.writeFileSync(file, content);
console.log('Success updating AttachmentManager.tsx');
