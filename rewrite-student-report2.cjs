...
import { Student, SubjectScore, SUBJECTS, CurriculumSubject } from "../types";
import {
  PDFPrintHelper,
  PrintPageContainer,
  PrintHeader,
        ...
        if (schoolSubjects.length > 0) {
           ...
           studentScores = parentsAndStandalone.map(subjectDef => {
             if (subjectDef.isParent) {
                ...
                return {
                   subject: subjectDef.subjectCode ? \`\${subjectDef.subjectCode} \${subjectDef.subjectName}\` : subjectDef.subjectName,
...
console.log("Rewrote StudentReportPrintTemplate again");
