...
# Find the start of extendedStudents
start_str = "const extendedStudents = React.useMemo(() => {"
start_idx = code.find(start_str)

if start_idx != -1:
    end_idx = code.find("  }, [initialStudent]);", start_idx)
    if end_idx != -1:
        end_idx += len("  }, [initialStudent]);")
        
        replacement = """  const extendedStudents = React.useMemo(() => {
    ...
    return list;
        ...
        with open('src/components/Student360.tsx', 'w') as f:
            f.write(code)
        print("Successfully replaced extendedStudents")
    else:
        print("Could not find end of useMemo")
else:
    print("Could not find start of useMemo")
