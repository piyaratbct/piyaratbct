...
const file = 'src/components/PBLLessonPlanForm.tsx';
...
code = code.replace(/  const subject = "บูรณาการ \(PBL\)";\n  const \[date, setDate\] = useState/, `  const subject = "บูรณาการ (PBL)";
  const teachingWeeks = generateTeachingWeeksOptions(systemAcademicYear);
...
// And in state init:
code = code.replace(/initialPlan\?\.date \|\| new Date\(\)\.toISOString\(\)\.slice\(0, 10\)/, `initialPlan?.date || teachingWeeks[0]`);
