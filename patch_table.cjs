const fs = require('fs');
const target = fs.readFileSync('target_block.txt', 'utf8');

const replacement = `          {curriculums.length === 0 ? (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3 mb-4 shadow-sm">
              <AlertCircle className="h-5 w-5 text-amber-500 mt-0.5 flex-shrink-0" />
              <div>
                <p className="text-sm font-bold text-amber-800">ไม่พบข้อมูลหลักสูตรสำหรับวิชาและชั้นเรียนที่เลือก</p>
                <p className="text-[11px] text-amber-700 mt-1">
                  ระบบจะไม่สามารถแสดงตัวชี้วัดคงเหลือ หรือเครื่องมือช่วยเลือกตัวชี้วัดได้ 
